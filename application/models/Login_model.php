...
class Login_model extends CI_Model
{
    ...
    {
        $this->db->select('BaseTbl.userId, BaseTbl.password, BaseTbl.name,BaseTbl.status,BaseTbl.roleId, Roles.role,Roles.state');
        $this->db->from('tbl_users as BaseTbl');
        $this->db->join('tbl_roles as Roles','Roles.roleId = BaseTbl.roleId');
        $this->db->where('BaseTbl.email', $email);
        $this->db->where('BaseTbl.isDeleted', 0);
        //$this->db->where('BaseTbl.status', 1);
        //$this->db->where('Roles.state', 1);
        //$this->db->query("Select * From tbl_users AS T1 Left Join tbl_roles AS T2 ON(T1.roleId = T2.roleId) Where T1.isDeleted = 0 and T1.status = 1 and T2.state = 1 and T1.email = '$email'");
        $query = $this->db->get();
        $user = $query->result();
